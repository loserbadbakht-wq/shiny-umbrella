...
const UNAME_INDEX_KEY = 'uname:index'; // { username -> { id, firstName, lastName } }
...
// ============= USERNAME INDEX =============
// username (lowercased) -> { id, firstName, lastName, username }
...
async function handleState(request, env) {
  ...
  // Remember username → id mapping for /hourstart lookups
  await indexUser(env, user);
...
export default {
  async fetch(request, env) {
    ...
    // Keep a fresh username mapping for whoever sends messages
...
async function handleUpdate(update, env, origin) {
  ...
  switch (cmd) {
    ...
    // ─── /migratedo — seed the DO from KV (admin, run once) ───
    case '/migratedo': {
    ...
    case '/debug': {
      ...
      if (env.BOT_KV) {
        ...
        lines.push('📇 Username index size');
        try {
