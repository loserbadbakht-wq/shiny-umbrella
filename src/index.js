...
// Display string for a stored hour target.
//   1. displayName set → ALWAYS plain text (no link, ever)
//   2. userId + name   → hyperlinked name via tg://user
//   3. userId + uname  → hyperlinked @username via tg://user
//   4. username only   → hyperlinked @username via t.me
//   5. fallback        → "User"
...
async function handleUpdate(update, env, origin) {
  ...
  switch (cmd) {
    // ─── /hourstart ───
    //   @username  → resolve + hyperlink
    //   anything else → stored as plain text, output is ALWAYS plain
    case '/hourstart': {
      ...
      if (rawInput.startsWith('@')) {
        ...
        // ── any plain text → ALWAYS plain text ──
        entry = {
          userId: null,
          firstName: '',
          lastName: '',
