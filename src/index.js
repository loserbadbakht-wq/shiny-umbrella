...
function buttonCaption(total, users) {
  const lines = [`انگشت های کل: ${total}`, ''];

  if (!users.length) {
    lines.push('هنوز کسی انگشت نزده');
  } else {
    const top = users.slice(0, BTN_TOP_LIMIT);
    for (const u of top) {
      lines.push(`انگشت های ${u.name}: ${u.count}`);
    }
    if (users.length > BTN_TOP_LIMIT) {
      lines.push(`و ${users.length - BTN_TOP_LIMIT} نفر دیگه...`);
    }
  }

  lines.push('');
  lines.push('اونی-چان انگشتم نکن!');
