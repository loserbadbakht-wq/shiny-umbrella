...
// This function now accepts a waitUntil callback
async function filterMediaHandler(ctx, waitUntil) {
    ...
    if (banned) {
        ...
        try {
            ...
            // Schedule deletion after 15 seconds using waitUntil
            ...
            if (waitUntil) {
                ...
                // Fallback: try without waitUntil (may not work)
                deleteTask();
            }
...
export default {
    async fetch(request, env, ctx) {
        ...
        // Media filter - wrap to pass waitUntil
        bot.on([':animation', ':sticker'], async (ctx) => {
            await filterMediaHandler(ctx, ctx.waitUntil);
        });
