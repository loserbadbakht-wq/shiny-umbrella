...
// /debug, /maltest, /schedule (native tables), and per-chat
// localized /command menus via BotCommandScopeChat.
// ============================================================
...
// ============= BOT COMMAND MENUS (localized) =============
const BOT_COMMANDS = {
    en: [
        { command: 'start', description: 'Subscribe to notifications' },
        { command: 'schedule', description: 'Weekly anime schedule' },
        { command: 'language', description: 'Change language' },
        { command: 'unsub', description: 'Unsubscribe from notifications' },
        { command: 'debug', description: 'Show latest 10 titles' },
        { command: 'maltest', description: 'Test MAL link resolver' },
    ],
    fa: [
        { command: 'start', description: 'عضویت در اطلاع‌رسانی' },
        { command: 'schedule', description: 'برنامه هفتگی انیمه‌ها' },
        { command: 'language', description: 'تغییر زبان' },
        { command: 'unsub', description: 'لغو اشتراک' },
        { command: 'debug', description: 'نمایش ۱۰ عنوان آخر' },
        { command: 'maltest', description: 'تست جستجوی لینک MAL' },
    ],
};

/**
 * Set the "/" command menu for a specific chat.
 *
 * Telegram's language_code parameter only matches the user's Telegram APP
 * language — not the bot's own language preference. So if we relied on it
 * alone, a user with an English Telegram app would never see the Persian
 * menu, even after pressing the فارسی button.
 *
 * To make the menu follow the bot's language choice, we register commands
 * with BotCommandScopeChat (per-chat scope). This overrides the default
 * scope and is independent of the Telegram app language.
 */
async function setBotCommandsForChat(env, chatId, lang) {
    const commands = lang === 'fa' ? BOT_COMMANDS.fa : BOT_COMMANDS.en;
    ...
    try {
        ...
        if (!json.ok) {
            console.warn(
                `[COMMANDS] setMyCommands (chat ${chatId}, ${lang}) failed:`,
                JSON.stringify(json).slice(0, 200)
            );
        } else {
            console.log(`[COMMANDS] Per-chat menu set for ${chatId} → ${lang}`);
        }
    } catch (e) {
        console.error('[COMMANDS] setMyCommands error:', e.message);
    }
}

/**
 * Also register the default (language_code-based) menus so that users
 * who haven't touched /language yet still get a sensible menu based on
 * their Telegram app language.
 */
async function setDefaultBotCommands(env) {
    // English — default fallback (no language_code).
    ...
    // Persian — served when the Telegram app itself is in Persian.
    await fetch(`${TELEGRAM_API}${env.BOT_TOKEN}/setMyCommands`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            commands: BOT_COMMANDS.fa,
            scope: { type: 'default' },
            language_code: 'fa',
        }),
    }).catch(() => {});
...
function formatTitle(rawTitle, lang = 'en', malLink = null) {
    ...
    const episodeMatch = !seasonMatch && title.match(/^(.*?)\s*[-–]\s*(\d+)\s*$/);

    let message;
    if (seasonMatch) {
        const base = seasonMatch[1].trim();
        const season = String(parseInt(seasonMatch[2], 10));
        const episode = String(parseInt(seasonMatch[3], 10));
        ...
        const base = episodeMatch[1].trim();
        const episode = String(parseInt(episodeMatch[2], 10));
        ...
        if (lang === 'fa') {
            message = `انیمه <b>${title}</b> اومد!`;
        } else {
            message = `<b>${title}</b> Aired!`;
        }
    }
...
async function handleStart(env, chatId) {
    ...
    // Ensure the per-chat menu matches the stored language.
    await setBotCommandsForChat(env, chatId, lang);
...
async function handleMalTest(env, chatId) {
    ...
    for (const s of samples) {
        ...
        const en = formatTitle(s, 'en', null);
        const fa = formatTitle(s, 'fa', null);
...
async function handleCallbackQuery(env, callbackQuery) {
    ...
    if (data && data.startsWith('lang:')) {
        ...
        // 2. Update the "/" command menu for THIS chat.
        await setBotCommandsForChat(env, chatId, lang);

        // 3. Confirm.
        const confirmText =
            lang === 'fa'
                ? '✅ زبان به فارسی تغییر کرد.\nدستورات ربات نیز به فارسی تغییر کرد.'
                : '✅ Language changed to English.\nBot commands updated too.';
...
export default {
    async fetch(request, env, ctx) {
        ...
        // Deploy-time hook: register default menus once.
        // Hit https://your-worker.workers.dev/setup after deploy.
        if (url.pathname === '/setup' && request.method === 'GET') {
            await setDefaultBotCommands(env);
            return new Response('Default command menus registered.', { status: 200 });
        }
