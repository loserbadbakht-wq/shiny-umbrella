...
// ============= UPDATED FILTER MEDIA HANDLER WITH WAITUNTIL =============
async function filterMediaHandler(ctx, waitUntil) {
    const chatId = ctx.chat.id;
    ...
    if (banned) {
        ...
        try {
            ...
            // Schedule deletion using waitUntil
            const deleteTask = async () => {
                await new Promise(resolve => setTimeout(resolve, 15000));
                try {
                    await ctx.api.deleteMessage(sentMessage.chat.id, sentMessage.message_id);
                    console.log('Warning message deleted after 15 seconds');
                } catch (e) {
                    console.log('Failed to delete warning message:', e.message);
                }
            };
            
            if (waitUntil) {
                waitUntil(deleteTask());
            } else {
                // Fallback
                deleteTask();
            }
...
export default {
    async fetch(request, env) {
        ...
        // Media filter with waitUntil support
        bot.on([':animation', ':sticker'], async (ctx) => {
            // Get the execution context from the fetch handler's waitUntil
            // We need to pass waitUntil from the outer scope
            // We'll store it in the bot instance
            const waitUntil = ctx.bot.waitUntil;
            await filterMediaHandler(ctx, waitUntil);
        });
