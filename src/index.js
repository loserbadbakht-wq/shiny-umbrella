...
// /debug, /maltest, /schedule using native Telegram tables,
// and per-chat command menus read from BotFather's localizations.
// ============================================================
...
// ============= BOTFATHER COMMAND CACHE =============
// We read the command lists that BotFather has stored (per language_code)
// and cache them in module scope so we only fetch once per isolate.
const commandCache = {
    en: null,
    fa: null,
};
...
/**
 * Fetch a language's command list from BotFather's stored commands.
 * @param {object} env
 * @param {'en'|'fa'} lang
 * @returns {Promise<Array<{command:string, description:string}>|null>}
 */
async function fetchBotFatherCommands(env, lang) {
    if (commandCache[lang]) return commandCache[lang];

    const payload = { scope: { type: 'default' } };
    // 'fa' → read the Persian localization; 'en' → read the default
    if (lang === 'fa') payload.language_code = 'fa';

    const url = `${TELEGRAM_API}${env.BOT_TOKEN}/getMyCommands`;
    try {
        ...
        commandCache[lang] = json.result;
        console.log(`[COMMANDS] Loaded ${json.result.length} ${lang} commands from BotFather`);
        return json.result;
    } catch (e) {
...
/**
 * Apply the given commands to a single chat, overriding whatever the
 * default (language_code-based) menu would show.
 */
async function setCommandsForChat(env, chatId, commands) {
    ...
    try {
        ...
        if (!json.ok) {
            console.warn(
                `[COMMANDS] setMyCommands (chat ${chatId}) failed:`,
                JSON.stringify(json).slice(0, 200)
            );
        } else {
            console.log(`[COMMANDS] Per-chat menu applied to ${chatId}`);
        }
    } catch (e) {
...
/**
 * Convenience: read BotFather's commands for the given language and
 * apply them to the chat.
 */
async function applyBotFatherCommands(env, chatId, lang) {
    const commands = await fetchBotFatherCommands(env, lang);
    if (commands) {
        await setCommandsForChat(env, chatId, commands);
    }
}
...
async function handleStart(env, chatId) {
    ...
    // Ensure the per-chat menu matches BotFather's commands for this language.
    await applyBotFatherCommands(env, chatId, lang);
...
async function handleCallbackQuery(env, callbackQuery) {
    ...
    if (data && data.startsWith('lang:')) {
        ...
        // 2. Read BotFather's commands for that language and apply them
        //    to THIS chat, overriding the default menu.
        await applyBotFatherCommands(env, chatId, lang);

        // 3. Confirm.
        const confirmText =
            lang === 'fa'
                ? '✅ زبان به فارسی تغییر کرد.\nمنوی دستورات نیز به فارسی تغییر کرد.'
                : '✅ Language changed to English.\nCommand menu updated too.';
