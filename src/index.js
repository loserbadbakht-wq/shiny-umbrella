...
function buttonText(total, name, userCount) {
  return `کلیک های کل: ${total}\nکلیک های ${name}: ${userCount}`;
}
...
async function handleCallbackQuery(update, env) {
  ...
  const newText = buttonText(newTotal, name, newUserCount);
  ...
  try {
    ...
    console.error('edit button msg failed:', e.message);
  }
  ...
  switch (cmd) {
    ...
    case '/button': {
      ...
      await sendMessage(
        env.BOT_TOKEN,
        msg.chat.id,
        buttonText(total, name, userCount),
        threadId ?? undefined,
        { reply_markup: buttonKeyboard() }
      );
