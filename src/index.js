 ...
 *
 * Board is rendered as a real PNG photo and edited in place each turn.
...
async function editBoardPhoto(env, chatId, messageId, pngBytes, caption) {
  const form = new FormData();
  form.append('chat_id', String(chatId));
...
function boardCaption(game) {
  ...
  if (game.phase === PHASE.MOVE) {
    return (
      ...
      `Pick a piece from the menu below.`
    );
  }
...
async function refreshBoard(env, game) {
  ...
  if (game.boardMessageId) {
    const res = await editBoardPhoto(env, game.chatId, game.boardMessageId, png, caption);
    if (res && res.ok) return;
    // The photo may have been deleted; fall through to send a new one.
    game.boardMessageId = null;
  }

  const sent = await sendBoardPhoto(env, game.chatId, png, caption);
  if (sent && sent.ok && sent.result) {
    game.boardMessageId = sent.result.message_id;
  }
}
...
/* ============================ BOARD RENDERER ============================ */
/* SVG kept text-free and filter-free for reliable resvg rendering.       */
...
function renderBoardSVG(game, size = 1000) {
  ...
  out.push(`<rect width="${size}" height="${size}" fill="#f8fafc"/>`);

  // Center home
  out.push(`<circle cx="${cx}" cy="${cy}" r="72" fill="#f1f5f9" stroke="#94a3b8" stroke-width="3"/>`);
  out.push(`<circle cx="${cx}" cy="${cy}" r="34" fill="#e2e8f0" stroke="#cbd5e1" stroke-width="2"/>`);
  out.push(`<circle cx="${cx}" cy="${cy}" r="14" fill="#cbd5e1"/>`);
  ...
  for (const p of game.players) {
    ...
    // Pieces (with offset shadow circle instead of SVG filter)
    for (let j = 0; j < RULES.piecesPerPlayer; j++) {
      const pos = p.pieces[j];
      let x, y;
      ...
      // Shadow
      out.push(
        `<circle cx="${f(x)}" cy="${f(y + pieceR * 0.28)}" r="${f(pieceR)}" ` +
      ...
      if (movable) {
        out.push(
          ...
          `stroke="#0f172a" stroke-width="2.5" stroke-dasharray="5 4" opacity="0.85"/>`
        );
      }
      ...
      out.push(
        ...
        `stroke="${movable ? '#0f172a' : '#ffffff'}" stroke-width="${movable ? 3 : 2}"/>`
      );
      // Highlight
      out.push(
        `<circle cx="${f(x - pieceR * 0.3)}" cy="${f(y - pieceR * 0.3)}" r="${f(pieceR * 0.24)}" ` +
        `fill="#ffffff" opacity="0.6"/>`
      );

      // Bot marker (center dot)
      if (p.isBot) {
        out.push(
          `<circle cx="${f(x)}" cy="${f(y)}" r="${f(pieceR * 0.28)}" ` +
...
export class GameRoom {
  ...
  async handleUpdate(update, workerUrl) {
    // Ignore updates from any bot (our own dice, our own replies).
    ...
    switch (cmd) {
      ...
      case '/help': {
        await sendMessage(env, chatId,
          ...
          `2. The bot offers legal pieces as buttons.\n` +
          `3. Roll a 6 to bring a piece out or to roll again.\n` +
          `4. First to bring all 4 pieces home wins.\n\n` +
          `The board photo updates automatically after every move.`
        );
        return undefined;
      }
      ...
      case '/board': {
        ...
        try {
          ...
          await sendBoardPhoto(env, chatId, png, boardCaption(game));
        } catch (e) {
          console.error('board cmd failed', e);
          await sendMessage(env, chatId, '⚠️ Could not render board.');
        }
        return undefined;
      }
  ...
  async onCallback(cq, game, workerUrl) {
    ...
    if (data.startsWith('mv:')) {
      ...
      // Clear buttons on the prompt message
      await editReplyMarkup(env, chatId, cq.message.message_id, []);
  ...
  async onDice(msg, game, workerUrl) {
    ...
    if (sent && sent.ok && sent.result) game.buttonsMessageId = sent.result.message_id;
  ...
  async sendTurnPrompt(game, workerUrl) {
    ...
    if (player.isBot) {
      // Bot loop will handle it — but only announce if this call came from a human context.
      return;
    }

    const extraTurn = false; // caller decides; here we just prompt the current player
    const text =
  ...
  async reportEvents(game, mover, move, events, workerUrl) {
    ...
    // Note: the standard "It's X's turn" prompt is sent by sendTurnPrompt
    // after the bot loop terminates.
  }
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET' && url.pathname.startsWith('/board/')) {
      ...
      // Serve PNG so the URL is directly usable in a browser too.
      try {
        const svg = renderBoardSVG(game, 1000);
        const png = await svgToPng(svg, BOARD_PNG_WIDTH);
