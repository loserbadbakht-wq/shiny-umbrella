...
// ============= FIXED LISTFILTERED HANDLER WITH PERSIAN SUPPORT =============
async function listFilteredHandler(ctx) {
    const chatId = ctx.chat.id;
    
    let lang;
    try {
        lang = await getUserLanguage(ctx.kv, ctx.from.id);
    } catch (e) {
        console.error('[ERROR] Failed to get language:', e);
        lang = 'en';
    }
    ...
    if (total === 0) {
        await ctx.reply(TEXTS[lang]?.empty_filter_list || 'Filter List is empty.');
        return;
    }
    ...
    try {
        ...
        // Build the message without markdown for Persian (or use HTML instead)
        let message = '';
        
        if (lang === 'fa') {
            // Persian version - simpler formatting without markdown
            message += `📋 فیلتر شده در ${groupName} (${total} عدد)\n\n`;
            message += `برای کپی کردن هر آیدی، روی آن فشار دهید و نگه دارید\n\n`;
            ...
            if (groupData.gifs && groupData.gifs.length > 0) {
                ...
                for (const item of groupData.gifs) {
                    try {
                        const date = item.added_date ? item.added_date.slice(0, 16).replace('T', ' ') : 'نامشخص';
            ...
            message += `💡 برای حذف: /unfilterowner <file_id>\n`;
            message += `برای کپی کردن هر آیدی، روی آن فشار دهید و نگه دارید`;
            
        } else {
            // English version with markdown
            message = `📋 **Filtered Media in ${groupName}** (${total} total)\n\n`;
        ...
        // Send with parse_mode only for English, plain text for Persian
        if (lang === 'fa') {
        ...
        // Ultimate fallback - simple text with just the count
        try {
