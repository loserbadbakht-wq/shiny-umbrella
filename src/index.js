...
// Display link:
//   - if we know the numeric user id → tg://user?id (opens chat)
//   - otherwise → https://t.me/<username> (opens public profile)
...
async function handleUpdate(update, env, origin) {
  ...
  switch (cmd) {
    ...
    case '/hourstart': {
      ...
      // Try to resolve from our index. If found → real name + id.
      // If not found → fall back to username-only (link to t.me/username).
