...
function buttonCaption(total, name, userCount) {
  return (
    `انگشت های کل: ${total}\n` +
    `انگشت های ${name}: ${userCount}\n` +
    `اونی-چان انگشتم نکن!`
  );
}
...
async function handleCallbackQuery(update, env) {
  ...
  await Promise.all([
    ...
    writeBtnUser(env, userId, { name, count: newUserCount }),
  ]);

  const newCaption = buttonCaption(newTotal, name, newUserCount);

  try {
    // Messages sent by /button always contain a photo → edit caption
...
async function handleUpdate(update, env, origin) {
  ...
  switch (cmd) {
    ...
    case '/button': {
      ...
      const [total, u] = await Promise.all([
        readBtnTotal(env),
        userId ? readBtnUser(env, userId) : Promise.resolve(null),
      ]);

      const userCount = u?.count || 0;

      await sendPhoto(
        env.BOT_TOKEN,
        msg.chat.id,
        BUTTON_IMAGE,
        buttonCaption(total, name, userCount),
        threadId ?? undefined,
        { reply_markup: buttonKeyboard() }
      );

      await logEvent(env, { ev: 'button_msg', total });
      return;
    }
