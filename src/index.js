...
async function handleUpdate(update, env, origin) {
  ...
  switch (cmd) {
    ...
    // ─── /hourend [force] ───
    //   /hourend         → only the target user can stop.
    //                      "Plain text" entries (free-form, no username, no userId)
    ...
    case '/hourend': {
      ...
      // "Plain text" ONLY when the entry was stored as free-form text.
      // Anything with a username (even unresolved) stays locked.
      const isPlainText =
