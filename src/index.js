...
// and per-chat command menus READ FROM BOTFATHER (never written).
// ============================================================
...
// Reads the command lists that BotFather stores, per language_code.
// Cached per isolate so we only fetch once per language per cold start.
const commandCache = {
    en: null,
    fa: null,
};
...
/**
 * Read BotFather's commands for a given language.
 * - lang='fa' → getMyCommands with language_code='fa'
 * - lang='en' → getMyCommands without language_code (the default list)
 *
 * This NEVER writes to BotFather's storage — it's read-only.
 */
...
/**
 * Apply BotFather's commands as a per-chat override, so the menu follows
 * the bot's own language choice instead of the user's Telegram app language.
 *
 * The commands themselves are copied verbatim from BotFather — this call
 * only affects the given chat and does NOT touch BotFather's storage.
 */
...
async function handleStart(env, chatId) {
    ...
    // Apply BotFather's commands (read-only fetch) as a per-chat override.
...
async function handleCallbackQuery(env, callbackQuery) {
    ...
    if (data && data.startsWith('lang:')) {
        ...
        //    as a per-chat override (does not modify BotFather).
        await applyBotFatherCommandsToChat(env, chatId, lang);
