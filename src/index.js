...
async function writeGame(env, game) {
  await env.BOT_KV.put('game', encryptData(game, env.DB_ENCRYPTION_KEY));
}
...
/* ── AMOLED button with rotating gradient border ── */
...
function render() {
  var total = serverState.total + pending;
...
function flush() {
  ...
  api('/api/tap', { count: n }).then(function(res) {
    ...
    serverState = res;
    render();
    if (pending > 0 && !timer) timer = setTimeout(flush, 250);
  }).catch(function() {
...
async function handleState(request, env) {
  ...
  const game = await readGame(env);
  return jsonResponse({
    total: game.total,
    users: game.users.slice(0, 50),
  });
}
...
async function handleTap(request, env) {
  ...
  const entry = game.users.find((u) => u.id === user.id);
  if (entry) {
    entry.count += n;
    entry.name = displayName;
  } else {
    game.users.push({ id: user.id, name: displayName, count: n });
  }

  game.users.sort((a, b) => b.count - a.count);
  ...
  switch (cmd) {
    ...
    case '/startgame': {
      ...
      await sendMessage(
        ...
        '🎮 برای شروع بازی روی دکمه زیر بزن:\n\n' +
          '(در صورت باز نشدن، ربات را در چت خصوصی استارت کن)',
        threadId ?? undefined,
        {
          reply_markup: {
            inline_keyboard: [
      ...
      await logEvent(env, { ev: 'startgame' });
      return;
    }
    ...
    case '/debug': {
      ...
      if (env.BOT_KV) {
        ...
        lines.push('🎮 Game');
        try {
          const g = await readGame(env);
          lines.push(`  total: ${g.total}`);
          lines.push(`  players: ${g.users.length}`);
        } catch (e) {
      ...
      lines.push('  /startgame  /start /end /force-end /list /test /ping /debug');
