...
const CRON_MIDNIGHT = '30 20 * * *'; // daily 00:00 Iran
const HOURLY_CRON   = '30 * * * *';  // every hour at :00 Iran
...
// ============= USERNAME INDEX (defensive — never throws) =============
...
// Never throws — indexing must never break message handling
...
function displayLink(entry) {
  ...
  const shown = name || entry.username || 'User';
  return `<a href="tg://user?id=${entry.id}">${htmlEsc(shown)}</a>`;
}
...
async function handleState(request, env) {
  try {
    ...
    console.error('handleState crashed:', e.stack || e.message);
...
async function handleTap(request, env) {
  try {
    ...
    console.error('handleTap crashed:', e.stack || e.message);
    return jsonResponse({ error: 'server error' }, 500);
  }
}

// ============= CALLBACK QUERY HANDLER (defensive) =============
async function handleCallbackQuery(update, env) {
  ...
  // Best-effort indexing (never breaks)
  try {
  ...
  try {
    ...
    console.error('callback crashed:', e.stack || e.message);
...
export default {
  async fetch(request, env) {
    ...
    // Pre-handler block: telemetry + indexing — NEVER throws
    try {
      if (msg?.text) {
      ...
      console.error('pre-handler failed:', e.message);
    }
...
async function handleUpdate(update, env, origin) {
  ...
  switch (cmd) {
    ...
    case '/hourstart': {
      ...
      const rawUname = parts[1].replace(/^@/, '');
      if (!/^[a-z0-9_]{3,32}$/i.test(rawUname)) {
        await sendMessage(
          env.BOT_TOKEN,
          msg.chat.id,
      ...
      // Resolve the username to a user ID + real name
      const found = await lookupUsername(env, rawUname);

      if (!found) {
        await sendMessage(
          env.BOT_TOKEN,
          msg.chat.id,
          `⚠️ کاربر @${rawUname} پیدا نشد.\n` +
            'کاربر باید حداقل یک بار ربات را استارت کرده یا دکمه را زده باشد.',
          threadId ?? undefined
        );
        return;
      }
      ...
      const entry = {
        ...
        username: found.username || rawUname,
      };
    ...
    case '/debug': {
        ...
        try {
          ...
          if (!hts.length) {
            ...
            hts.forEach((t, i) => {
              ...
              const nm = [t.firstName, t.lastName].filter(Boolean).join(' ');
      ...
      lines.push('  /hourstart @username  /hourend');
