...
// ── Stable index of user IDs (fixes "only see myself" lag) ──
...
// ── Lord keys (keyed by lowercase username) ──
...
// ── List users (merge index + KV list) ──
async function listUsers(env) {
  const idSet = new Set();

  // 1) Index (usually fresh)
  for (const id of await readIndex(env)) idSet.add(String(id));

  // 2) KV list (authoritative but eventually consistent)
  let cursor;
  do {
...
async function handleTap(request, env) {
  ...
  // Add to stable index so other users discover this player quickly
...
async function handleUpdate(update, env, origin) {
  ...
  switch (cmd) {
    ...
    case '/debug': {
      ...
      lines.push('  /startgame /start /end /force-end /list /players /test /ping /debug');
