...
export class ButtonCounter {
  ...
  async fetch(request) {
    ...
    // ─── POST /seed ─── one-time migrate from KV
    if (url.pathname === '/seed') {
      ...
      if (
        ...
        // Only seed if DO is empty (don't overwrite live data)
...
// ============= BUTTON COUNTER (KV fallback + seed source) =============
...
async function readBtnState(env) {
  ...
  // Migrate from old per-user format
...
// Helper: read state from DO, fall back to KV on error
...
// ============= CALLBACK QUERY HANDLER (DO-routed) =============
async function handleCallbackQuery(update, env) {
  ...
  try {
    // Route the tap through the Durable Object (atomic)
    const id = env.BTN_COUNTER.idFromName('global');
    const stub = env.BTN_COUNTER.get(id);
...
export default {
  ...
  async scheduled(event, env) {
    ...
    if (event.cron !== CRON_MIDNIGHT) return;
