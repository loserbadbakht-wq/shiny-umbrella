...
// Entire leaderboard stored as ONE encrypted blob → 1 read + 1 write per tap
...
async function readBtnState(env) {
  ...
  // Migrate from old per-user format (one-time)
...
async function migrateBtnState(env) {
  ...
  // If we found nothing, start clean
  if (!users.length && !total) return { total: 0, users: [] };

  const migrated = { total, users };

  // Persist the new blob so future taps use it
  try {
...
// Caption for /button:
//   - lords:     hyperlinked name + «title»
//   - non-lords: plain text name, no link
...
// Plain-text fallback (no HTML, no hyperlinks)
...
// Never cuts HTML tags mid-way — drops whole lines instead
...
async function handleCallbackQuery(update, env) {
  ...
  // Always ack the callback first so the button spinner stops
  try {
    await answerCallbackQuery(env.BOT_TOKEN, cq.id);
  } catch { /* ignore */ }

  // Log every callback we receive so /debug shows it
  await logEvent(env, {
    ev: 'cb_recv',
    data: cq.data || '(none)',
  ...
  if (cq.data !== 'btn:click' && cq.data !== 'btn:click:lord') {
    await logEvent(env, { ev: 'cb_ignore', data: cq.data });
    return;
  }
  ...
  try {
    // Read the single blob
    const state = await readBtnState(env);

    // Increment
    state.total = (state.total || 0) + 1;
    const entry = state.users.find((u) => u.id === userId);
    if (entry) {
      entry.count += 1;
      entry.name = name;
      if (username) entry.username = username;
    } else {
      state.users.push({ id: userId, name, username, count: 1 });
    }
    state.users.sort((a, b) => b.count - a.count);
    if (state.users.length > 100) state.users.length = 100;

    await writeBtnState(env, state);

    // Enrich with lord titles
    const enriched = [];
    ...
    // Try HTML edit, then fallback to plain text
    let editStatus = 'ok';
    try {
    ...
    await logEvent(env, {
      ev: 'btn_click',
      total: state.total,
      user: entry ? entry.count : 1,
      edit: editStatus,
    });
  } catch (e) {
    console.error('callback handler crashed:', e.stack || e.message);
    await logEvent(env, { ev: 'callback_crash', msg: e.message });
  }
}
...
export default {
  async fetch(request, env) {
    ...
    if (msg?.text) {
      await logEvent(env, { ev: 'update', cmd: parseCommand(msg.text) });
    }
...
async function handleUpdate(update, env, origin) {
  ...
  switch (cmd) {
    // ─── /button — send the button message ───
    case '/button': {
      const state = await readBtnState(env);
    ...
    case '/buttonlord': {
      ...
      if (parts.length < 2) {
        await sendMessage(
          ...
          msg.chat.id,
          'ℹ️ استفاده:\n' +
            '`/buttonlord @username متن لقب` — ارتقا به لرد\n' +
            '`/buttonlord @username` — حذف لقب',
          threadId ?? undefined
        );
        return;
      }
    ...
    case '/debug': {
      ...
      if (env.BOT_KV) {
        ...
        try {
          const bs = await readBtnState(env);
          const lordable = bs.users.filter((u) => u.username).length;
          lines.push(`  total clicks: ${bs.total}`);
          lines.push(`  tappers: ${bs.users.length}`);
          lines.push(`  lordable: ${lordable}`);
        } catch (e) {
      ...
      try {
        ...
        if (log.length === 0) {
          ...
          for (const e of log.slice(0, 10)) {
            ...
            if (e.chatType) parts.push(`chatType=${e.chatType}`);
            if (e.edit) parts.push(`edit=${e.edit}`);
            if (e.hasMsg != null) parts.push(`hasMsg=${e.hasMsg}`);
      ...
      lines.push('  /buttonlord @username <text>  (admin, promote)');
      lines.push('  /buttonlord @username  (admin, demote)');
