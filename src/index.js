...
async function readBtnState(env) {
  const raw = await env.BOT_KV.get(BTN_STATE_KEY, { cacheTtl: 0 });
  if (raw) {
    try {
...
async function migrateBtnState(env) {
  ...
  // Old total
  const rawTotal = await env.BOT_KV.get('btn:total', { cacheTtl: 0 });
  if (rawTotal) {
    try {
  ...
  do {
    ...
    for (const k of res.keys) {
      ...
      const raw = await env.BOT_KV.get(k.name, { cacheTtl: 0 });
      if (!raw) continue;
      try {
        const u = decryptData(raw, env.DB_ENCRYPTION_KEY);
...
async function handleCallbackQuery(update, env) {
  ...
  try {
    ...
    // Enrich with lord titles (cheap — only for the top list)
    const enriched = [];
    ...
    const newCaption = buttonCaption(state.total, enriched);
    ...
    try {
      ...
      // These are harmless — KV was already updated
      if (!/not modified|too many requests|429/i.test(m)) {
        console.error('edit caption failed:', m);
      }
    }
