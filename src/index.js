...
// Direct Link WebApp — works in groups, channels, and private chats
...
async function listUsers(env) {
  ...
  do {
    ...
    for (const k of res.keys) {
      const id = k.name.slice(USER_PREFIX.length);
...
function render() {
  ...
  var list = serverState.users.map(function(u) {
    if (u.id === myId) myServerCount = u.count;
    return {
      id: u.id,
      name: u.name,
  ...
  if (pending > 0 && myId && !list.some(function(u) { return u.id === myId; })) {
    var meName = (tg && tg.initDataUnsafe && tg.initDataUnsafe.user)
      ...
      : 'You';
    list.push({ id: myId, name: meName, count: pending, isMe: true });
  }
...
async function handleTap(request, env) {
  ...
  await writeUser(env, user.id, { name: displayName, count: newCount });

  // Build the response. We can't trust listUsers() to reflect the write we
  // just made (KV is eventually consistent), so we force-override our own
  // entry with the authoritative value we just computed.
  const users = await listUsers(env);

  const idx = users.findIndex((u) => u.id === user.id);
  if (idx >= 0) {
    users[idx] = { id: user.id, name: displayName, count: newCount };
  } else {
    users.push({ id: user.id, name: displayName, count: newCount });
  }
  users.sort((a, b) => b.count - a.count);
...
async function handleUpdate(update, env, origin) {
  ...
  switch (cmd) {
    ...
    case '/resetgame': {
      if (msg.from?.id !== ADMIN_ID) {
        await sendMessage(
          env.BOT_TOKEN,
          msg.chat.id,
          '⛔ فقط ادمین میتواند بازی را ریست کند.',
          threadId ?? undefined
        );
        return;
      }
      ...
      await env.BOT_KV.delete('game');
      await logEvent(env, { ev: 'resetgame', deleted });
      await sendMessage(
        env.BOT_TOKEN,
        msg.chat.id,
        `🗑 بازی ریست شد. ${deleted} بازیکن پاک شد.`,
        threadId ?? undefined
      );
      return;
    }
    ...
    case '/debug': {
      ...
      try {
        ...
        if (log.length === 0) {
          ...
          for (const e of log.slice(0, 10)) {
            ...
            if (e.deleted != null) parts.push(`deleted=${e.deleted}`);
      ...
      lines.push('  /startgame /resetgame /start /end /force-end /list /test /ping /debug');
