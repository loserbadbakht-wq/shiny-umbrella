...
// Build the hourly message.
// The "you can stop it" hint is only sent when the target is a LINKED user,
// because plain-text targets can be stopped by anyone.
...
async function handleUpdate(update, env, origin) {
  ...
  switch (cmd) {
    ...
    // ─── /hourend [force] ───
    //   /hourend         → only the target user can stop (plain text = anyone)
    //   /hourend force   → admin-only, bypasses the check
    case '/hourend': {
      const parts = msg.text.trim().split(/\s+/);
      ...
      if (isForce) {
        if (senderId !== ADMIN_ID) {
          ...
          await sendMessage(
            env.BOT_TOKEN,
            msg.chat.id,
            `${RLM}تو (${htmlEsc(senderName)}) نیستی، فقط اون می‌تونه وقتی برگشت متوقف کنه ساعت شمار رو`,
            threadId ?? undefined,
            { parse_mode: 'HTML', link_preview_options: { is_disabled: true } }
          );
          await logEvent(env, {
      ...
      // ─── Normal mode: target-only (plain = anyone) ───
      const allowed = isPlain || target.userId === senderId;
    ...
    case '/test': {
      ...
      if (dt) {
        ...
        await sendMessage(
          env.BOT_TOKEN,
          dt.chatId,
          `🧪 (test daily) ${MSG_LINE_1} ${(dt.counter || 0) + 1}\n${MSG_LINE_2}`,
      ...
      if (!sentAny) {
        await sendMessage(
          env.BOT_TOKEN,
          msg.chat.id,
          '⚠️ نه روز شمار فعاله نه ساعت شمار.\n' +
            'برای روز شمار: `/start`\n' +
