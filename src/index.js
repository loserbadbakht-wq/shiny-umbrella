...
// Every tapper's name is hyperlinked to their profile.
// Lords additionally get "آقا سگه" prefix and "«title»" suffix.
  ...
  if (!users.length) {
    ...
    for (const u of top) {
      ...
      const namePart = u.lord
        ? `آقا سگه ${linkedName} «${htmlEsc(u.lord)}»`
        : linkedName;
...
async function handleCallbackQuery(update, env) {
  ...
  // Accept both legacy ('btn:click:lord') and current ('btn:click')
  ...
  const newCaption = buttonCaption(newTotal, allUsers);
