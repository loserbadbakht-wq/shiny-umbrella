...
// /debug, /maltest, /schedule (native tables), and localized
// /command menus via setMyCommands.
// ============================================================
...
// These are shown in the "/" menu. language_code = 'fa' for Persian;
// omitting language_code (or empty string) registers the default (English).
const BOT_COMMANDS = {
    en: [
...
/**
 * Register the bot command menu for a given language.
 * - lang = 'fa' → Persian menu (language_code = 'fa')
 * - lang = 'en' → default menu (no language_code, acts as fallback)
 */
async function setBotCommands(env, lang) {
    ...
    // Only set language_code for non-default languages. Omitting it
    // registers the fallback that Telegram shows for all other locales.
    if (lang === 'fa') {
        payload.language_code = 'fa';
    }
    ...
    try {
        ...
        if (!json.ok) {
            console.warn(
                `[COMMANDS] setMyCommands failed for ${lang}:`,
                JSON.stringify(json).slice(0, 200)
            );
        } else {
            console.log(`[COMMANDS] Menu set for ${lang}`);
        }
    } catch (e) {
...
async function handleStart(env, chatId) {
    ...
    // Make sure the "/" menu matches the stored language.
    await setBotCommands(env, lang);
...
async function handleCallbackQuery(env, callbackQuery) {
    ...
    if (data && data.startsWith('lang:')) {
        ...
        // 2. Update the "/" command menu immediately.
        await setBotCommands(env, lang);
