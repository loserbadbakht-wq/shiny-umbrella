...
// ============= UPDATED FILTER MEDIA HANDLER WITH AUTO-DELETE =============
async function filterMediaHandler(ctx) {
    ...
    if (banned) {
        ...
        try {
            ...
            // Schedule deletion after 15 seconds using the bot's API
            const bot = ctx.bot; // Get the bot instance
            const chatIdToDelete = sentMessage.chat.id;
            const messageIdToDelete = sentMessage.message_id;
            
            setTimeout(async () => {
                try {
                    await bot.api.deleteMessage(chatIdToDelete, messageIdToDelete);
