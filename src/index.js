...
// Rich Message bot.
//
//  ▸ Channels (bot is admin): auto-edits any post whose text contains at
//    least one HTML tag, turning it into a Rich Message. Posts without
//    tags are ignored. Posts by other bots are ignored.
//
//  ▸ Groups / DMs: /rich command flow (reply to HTML or inline HTML).
//
//  ▸ /ping, /debug, /help available in groups and DMs.
...
async function handleHelp(message, env) {
  await sendPlain(env, message.chat.id, message.message_id,
    ...
    '<b>In channels</b> (bot must be admin with "Edit messages" right)\n' +
    'Post anything containing HTML tags — the bot edits it into a Rich Message automatically.\n\n' +
    '<b>In groups / DMs</b>\n' +
    '<code>/rich</code> — reply to an HTML message, or pass HTML as the argument\n' +
...
async function handleRich(message, env, args, botId) {
  ...
  // Guard: never echo the bot's own rich messages
  ...
  if (args) {
    ...
    await sendPlain(env, message.chat.id, replied.message_id,
      ...
      `kind: <code>${escapeHtml(fromReplied.kind)}</code>`,
      threadId);
    return;
  } else {
    await sendPlain(env, message.chat.id, message.message_id,
      '⚠️ <b>Nothing to convert.</b>\n' +
      'Reply to a message containing HTML, or use <code>/rich &lt;b&gt;Hello&lt;/b&gt;</code>.',
      threadId);
    return;
  }
  ...
  console.warn('sendRichMessage failed → plain fallback:', rich.description);
...
async function handleChannelPost(post, env, isEdit) {
  ...
  if (DEBUG) console.log(`channel ${isEdit ? 'edit' : 'post'} [${chatId}:${msgId}]`);

  // Already a rich message → nothing to do (also prevents loops)
  if (post.rich_message) return;

  // Skip posts authored by any bot (including ourselves)
  if (post.from?.is_bot) return;

  // Only plain text posts (not captions of media)
  const text = post.text;
  if (!text) return;

  // Skip if there is no HTML tag anywhere
  if (!hasHtmlTag(text)) return;

  // Convert: replace the post body with a rich message
  ...
  if (!r.ok) {
    console.error(`channel edit failed [${chatId}:${msgId}]:`, r.description);
  } else if (DEBUG) {
    console.log(`channel rich edit ok [${chatId}:${msgId}]`);
  }
}
    ...
    if (request.method === 'POST' && url.pathname === '/webhook') {
      ...
      try {
        if (update.channel_post) {
          await handleChannelPost(update.channel_post, env, false);
        } else if (update.edited_channel_post) {
          await handleChannelPost(update.edited_channel_post, env, true);
