...
// Lord caption — the lord's name is hyperlinked to their profile.
// Requires parse_mode: 'HTML' on the sendPhoto / editMessageCaption call.
function buttonLordCaption(total, users) {
  ...
  if (!users.length) {
    ...
    for (const u of top) {
      ...
      // Only users with a lord title get their name hyperlinked
      const namePart = u.lord
        ? `<a href="tg://user?id=${u.id}">${safeName}</a>`
        : safeName;
      const lordPart = u.lord ? ` «${htmlEsc(u.lord)}»` : '';
      lines.push(`${RLM}انگشت های ${namePart}${lordPart}: ${u.count}`);
    }
    if (users.length > BTN_TOP_LIMIT) {
      lines.push(`${RLM}و ${users.length - BTN_TOP_LIMIT} نفر دیگه...`);
    }
