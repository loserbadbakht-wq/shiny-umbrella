...
// and /schedule (Persian, Asia/Tehran time).
// ============================================================

const RSS_URL = 'https://subsplease.org/rss/?t&r=1080';
const TELEGRAM_API = 'https://api.telegram.org/bot';

// ---- Schedule uses Asia/Tehran timezone ----
const SCHEDULE_TZ = 'Asia/Tehran';
const SCHEDULE_API = `https://subsplease.org/api/?f=schedule&tz=${encodeURIComponent(SCHEDULE_TZ)}`;
...
// Persian full day names (header + buttons) — always used for /schedule.
const DAY_FULL_FA = {
    Monday: 'دوشنبه',
    Tuesday: 'سه‌شنبه',
    Wednesday: 'چهارشنبه',
...
/**
 * Get the current day name in Iran time (Asia/Tehran).
 * Uses Intl.DateTimeFormat with timeZone option to derive the weekday.
 */
function getCurrentDayIran() {
...
/**
 * Format a day's schedule in Persian. Times come from the API already
 * in Asia/Tehran time (because of the `tz` param), so no conversion.
 */
function formatDaySchedule(day, entries) {
    const dayName = DAY_FULL_FA[day] || day;

    let msg = `📅 <b>${dayName}</b> (به وقت ایران)\n\n`;

    if (!entries || entries.length === 0) {
        msg += '<i>هیچ انتشار برنامه‌ریزی‌شده‌ای وجود ندارد.</i>';
        return msg;
    }
...
/**
 * Build the Persian day keyboard. Callback data stays English so the
 * API keys keep working; only the labels and order are Persian.
 */
function buildDayKeyboard(activeDay) {
    const buttons = DAYS_FA_ORDER.map((d) => ({
        text: d === activeDay ? `⭐ ${DAY_FULL_FA[d]}` : DAY_FULL_FA[d],
...
/**
 * /schedule — Persian-only, Asia/Tehran time. Shows today's schedule
 * with 7 Persian day buttons underneath.
 */
async function handleSchedule(env, chatId) {
    try {
        const schedule = await fetchSchedule();
        const today = getCurrentDayIran();
        ...
        await sendMessage(env, chatId, `📅 <b>خطای برنامه:</b> ${e.message}`);
    }
}
...
async function handleCallbackQuery(env, callbackQuery) {
    ...
    // ----- Schedule day buttons (always Persian) -----
